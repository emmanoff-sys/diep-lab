...
def locate(nodes: list[dict], edges: list[dict], fault_current_a: float | None = None,
           outage_nodes: list[str] | None = None, options: dict | None = None) -> dict:
    """Locate a fault from a measured fault current and/or outage-reporting nodes."""
    net = build_radial(nodes, edges)
    root, by_id, energized, pu = net["root"], net["by_id"], net["energized"], net["pu"]
    ...
    if fault_current_a and fault_current_a > 0:
        for nid in net["order"]:
            ...
            impedance_candidates.append({
                "section": parent_edge[nid]["edge_id"], "to_node": nid,
    ...
    if outage_nodes:
        ...
        for nid in net["order"]:
            ...
            if best is None or jacc > best["score"]:
                best = {"section": parent_edge[nid]["edge_id"], "isolates_subtree": sorted(sub),
                        "score": round(jacc, 3),
    ...
    if topological and topological.get("matches_report"):
        ...
        best_estimate = {"method": "topological+impedance", "section": topological["section"],
                         "distance_km": (in_lateral[0]["distance_km"] if in_lateral else None)}
    elif impedance_candidates:
        best_estimate = {"method": "impedance", "section": impedance_candidates[0]["section"],
                         "distance_km": impedance_candidates[0]["distance_km"]}
    elif topological:
        best_estimate = {"method": "topological", "section": topological["section"],
                         "distance_km": None}

    return {
        "method": "impedance reactance-to-fault + topological last-gasp",
        "inputs": {"fault_current_a": fault_current_a,
