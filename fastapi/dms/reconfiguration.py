...
def _evaluate(nodes: list[dict], edges: list[dict], loads: dict) -> dict | None:
    ...
    if not res["converged"]:
        return None
    max_loading = max((b["loading_pct"] or 0.0) for b in res["branches"]) if res["branches"] else 0.0
    return {
...
def recommend(nodes: list[dict], edges: list[dict], loads: dict,
    ...
    for combo in itertools.product([False, True], repeat=len(sw_ids)):
        ...
        for eid, closed in zip(sw_ids, combo):
            cbi[eid]["is_closed"] = closed
        m = _evaluate(nodes, cand_edges, loads)
        if m is None:
            continue
        feasible_count += 1
        combo_map = dict(zip(sw_ids, combo))
    ...
    improvement = (round(cur_loss - best["loss_kw"], 3)
                   if (current and best) else None)
    improvement_pct = (round(100.0 * improvement / cur_loss, 1)
                       if (improvement is not None and cur_loss) else None)
    ...
    return {
        ...
        "evaluated": evaluated, "feasible_count": feasible_count,
        "current": current,
        "recommended": best,
        "switching_plan": plan,
