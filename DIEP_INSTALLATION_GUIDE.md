...
### 6.1 MQTT mTLS (mandatory, already provisioned in the lab CA)

| Artifact | Location (lab) | Purpose |
|---|---|---|
| CA certificate | `certs/devices/ca.crt` | Trust root for the Mosquitto broker and all device/service client certs |
| Broker cert/key | `mosquitto/config/certs/server.crt` / `server.key` | Server identity for the 8883 mTLS listener |
| Per-device client certs | `certs/devices/<DEVICE_ID>.crt` / `.key` (e.g. `BAT001`, `EV001`, `INV001`, `MG001`, `METER001`, plus `ingestor`, `dispatcher`, `csms`) | Client identity (CN) used by `mosquitto.conf`'s `use_identity_as_username` for ACL binding |

For a customer pilot:
1. Generate (or extend) the CA for the pilot's device fleet — one client cert per device
   ID, plus `ingestor`/`dispatcher`/`csms` (OCPP CSMS) service certs.
2. Distribute device certs to edge gateways via a secure out-of-band channel (not MQTT).
3. Update `mosquitto/config/acl` to scope each new device CN to its own topic namespace
...
- [ ] CA + per-device mTLS certs generated and placed under `certs/devices/`
- [ ] `mosquitto/config/acl` updated for the device fleet
- [ ] `.env` populated from `.env.example` with **all** secrets rotated (see
      [`PHASE15A_SECURITY_HARDENING_REPORT.md`](PHASE15A_SECURITY_HARDENING_REPORT.md) —
      note `DIEP_ADMIN_PASSWORD`/`DIEP_OPERATOR_PASSWORD`/`DIEP_VIEWER_PASSWORD`/
      `DIEP_ACME_PASSWORD`/`DIEP_GLOBEX_PASSWORD` were **not** rotated in the lab and must
      be before go-live)
