"""Ingestor health + metrics HTTP endpoint — stdlib http.server only, same
lightweight pattern as services/opcua/health.py (no new web-framework
dependency). /health reports queue depth and worker count for a quick
eyeball without a Prometheus query.
"""
...
logger = logging.getLogger("diep-ingestor.health")


class _Handler(BaseHTTPRequestHandler):
    server_version = "diep-ingestor-health/1.0"
    ...
    def do_GET(self):
        if self.path == "/health":
            ...
            self._respond_json(200, {"status": "UP", "service": "ingestor", **body})
...
def start_health_server(port: int, status_provider=None) -> ThreadingHTTPServer:
    ...
    thread = threading.Thread(target=server.serve_forever, daemon=True, name="ingestor-health")
    thread.start()
