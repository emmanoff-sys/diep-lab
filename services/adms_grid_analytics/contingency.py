...
def _is_radial(nodes: list[dict], edges: list[dict], energized: set) -> bool:
    ...
    ec = sum(
        1
        for e in edges
        if e.get("is_closed", True) and e["from_node"] in energized and e["to_node"] in energized
    )
...
def _restore(nodes: list[dict], edges: list[dict], failed_eid: str, lost: set) -> list[str]:
    ...
    openable = [
        e
        for e in edges
        if not e.get("is_closed", True) and e.get("is_switchable") and e["edge_id"] != failed_eid
    ]
    progress = True
...
def analyze(
    ...
    candidates = [
        e
        for e in edges
        if e.get("is_closed", True) and e["edge_type"] in ("line", "transformer", "switch", "tie")
    ]
    ...
    for ce in candidates:
        ...
        unserved_nodes = [
            n for n in still_out if classif_load(n) > 0 or customers_by_node.get(n, 0) > 0
        ]
        ...
        results.append(
            {
                "element": ce["edge_id"],
                "element_type": ce["edge_type"],
                "lost_load_kw": lost_load,
                "lost_generation_kw": lost_gen,
