...
def estimate(
    nodes: list[dict], edges: list[dict], measurements: dict, options: dict | None = None
) -> dict:
    ...
    for nid in state_nodes:
        ...
        rowp[ip[nid]] = 1.0
        H.append(rowp)
        z.append(float(base.get("base_load_kw") or 0.0))
        ...
        rowq[iq[nid]] = 1.0
        H.append(rowq)
        z.append(float(base.get("base_load_kvar") or 0.0))
    ...
    if norm_resid:
        ...
        if norm_resid[bi] > opt["bad_data_threshold"]:
            bad = {
                **meta[bi],
                "normalized_residual": round(norm_resid[bi], 2),
    ...
    for n in nodes:
        ...
        if nid == root:
            ...
            var_v = sum(hrow[a] * Ginv[a][b] * hrow[b] for a in range(ncol) for b in range(ncol))
        ...
        node_results.append(
            {
                ...
                "estimated_p_kw": round(p_est, 2),
                "estimated_q_kvar": round(q_est, 2),
