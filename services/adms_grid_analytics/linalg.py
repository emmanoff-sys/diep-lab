...
def transpose(a: list[list[float]]) -> list[list[float]]:
    return [list(col) for col in zip(*a, strict=False)] if a else []


def matmul(a: list[list[float]], b: list[list[float]]) -> list[list[float]]:
    bt = transpose(b)
    return [[sum(ai * bj for ai, bj in zip(row, col, strict=False)) for col in bt] for row in a]
...
def inverse(a: list[list[float]]) -> list[list[float]]:
    ...
    for col in range(n):
        ...
        for r in range(n):
            ...
            factor = m[r][col]
            if factor != 0.0:
                m[r] = [vr - factor * vc for vr, vc in zip(m[r], m[col], strict=False)]
