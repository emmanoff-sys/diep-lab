...
def solve(nodes: list[dict], edges: list[dict], loads: dict, options: dict | None = None) -> dict:
    ...
    parent_edge, subtree, pu = net["parent_edge"], net["subtree"], net["pu"]
    ...
    branch_phase = {
        eid: [p for p in node_phases[child_of[eid]] if p in edge_phases[eid]] for eid in child_of
    }
    ...
    for n in nodes:
        ...
        if nid not in energized:
            node_results.append(
                {"node_id": nid, "name": n.get("name"), "energized": False, "phases": {}}
            )
            continue
        ...
        vuf = (
            round(100.0 * max(abs(m - vavg) for m in mags) / vavg, 2)
            if vavg and len(mags) == 3
            else None
        )
        node_results.append(
            {
