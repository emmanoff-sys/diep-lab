...
|---|---|---|
| MW1 — K1 PITR + K4 Redis Sentinel | SEC-1, SEC-2, SEC-3, SEC-4, SEC-5, MON-1, MON-2, MON-3, MON-4, INFRA-2 | 🟢 Executed 2026-06-17; **SEV-1 regression during soak (MW1 PITR WAL Staging Leak), recovered 2026-06-18.** K1 wal-shipper never pruned local WAL → disk filled → PostgreSQL crash-loop. Fixed (`ship-wal.sh` now upload→verify→prune + 2 GiB staging alarm), PITR chain preserved (only MinIO-confirmed segments deleted), recovery validated live. K4 (Redis Sentinel) was unaffected and remains healthy. Full incident detail in `MW1_OUTAGE_RECOVERY_REPORT.md`; `MW1_VERIFICATION_REPORT.md` §12. **Soak INVALIDATED and restarted — fresh 48h from 2026-06-18 10:25Z.** |
| MW2 — K6 MinIO HA | MW1 complete + 48hr soak passed | 🔴 (MW1 recovered but on a fresh soak; earliest eligibility 2026-06-20 10:25Z) |
...
| 2026-06-18 (MW1 executed 06-17, then SEV-1 during soak) | MW1 cutover executed (K1 PITR + K4 Sentinel, commit `0ceb9e7`); then **MW1 PITR WAL Staging Leak** incident — recovered | All 11 gating items remain 🟢/🟡 as of 06-17; this row is operational, not item-status. SEV-1: wal-shipper never pruned local WAL staging → 48 GB disk filled → PostgreSQL crash-loop (26 restarts) → FastAPI `database:false`. Redis/Sentinel unaffected. | Recovered: deleted only 279 MinIO-confirmed WAL segments (30 unshipped preserved+since-shipped, PITR chain intact); `ship-wal.sh` rewritten to upload→verify→prune + 2 GiB staging alarm; PostgreSQL + FastAPI healthy; validated live. **MW1 soak INVALIDATED → fresh 48h from 2026-06-18 10:25Z (earliest MW2 2026-06-20 10:25Z).** See `MW1_OUTAGE_RECOVERY_REPORT.md`. |
| | | | | |
